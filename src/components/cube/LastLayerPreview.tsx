...
import { CubeTile, PreviewArrows, PuzzleColor } from "../../types";
import { puzzleColors } from "../../constants/puzzles";

type LastLayerPreviewProps = {
  previewString: string;
  theme: Theme;
  previewArrows: PreviewArrows;
};

const LastLayerPreview = ({
  previewString,
  previewArrows,
  theme,
}: LastLayerPreviewProps) => {
  const cubeSize = 3;
  ...
  const [top, right, bottom, left, firstRow, secondRow, lastRow] = previewString
    .split(" ")
    .map((str) => str.split("") as PuzzleColor[]);
