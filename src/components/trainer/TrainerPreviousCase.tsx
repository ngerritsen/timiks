...
import { retryCase, reQueueCase } from "../../slices/trainer";
import {
  getTrainingType,
  getLastCase,
            ...
            <Section>
                ...
                (!lastCase || lastCase.id !== currentCaseId) && (
                  <>
                    <Shortcut
                      ...
                      action={() =>
                        dispatch(
                          retryCase({ trainingType, caseId: lastCase.id })
                        )
                      }
                    />
                    <Link
              ...
              {isInRehearsal && !isQueued && (
                <>
                  <Shortcut
                    command="reQueueCase"
                    action={() => dispatch(reQueueCase(lastCase.id))}
                  />
                  <Link
                    href="#"
                    onClick={(event) => {
                      event.preventDefault();
                      dispatch(reQueueCase(lastCase.id));
                    }}
                  >
