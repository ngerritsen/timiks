...
const sourceOptions = importSources.map((source) => ({
  label: source.title,
  value: source.type,
}));
      ...
      content={({ hide }) => (
        <Modal title="Import" onClose={hide}>
          <>
            ...
            <Section margin={times ? "lg" : undefined}>
              <FileUploadButton
            ...
            {times && times.length === 0 && (
              <Warning>No valid times found in file.</Warning>
            )}
            {times && times.length > 0 && (
              <>
                <Section margin="xs">
                ...
                <Section margin="md">
                  <Button
                    disabled={!puzzle}
                    onClick={() => {
                      dispatch(importTimes(addPuzzleToTimes(times, puzzle)));
                      hide();
                    }}
                  >
                    Import
